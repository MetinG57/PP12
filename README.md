...
1. **What steps are required to open an X11 window and receive events?**
 ```
   1.Verbindung zum X-Server herstellen mit XOpenDisplay(NULL).
   2.Den Standardbildschirm abfragen mit DefaultScreen().
   3. Ein einfaches Fenster erstellen (Größe, Position, Farben) mit
   XCreateSimpleWindow().
   4. Festlegen, welche Ereignisse das Fenster empfangen soll, z. B. Tastendrücke oder
   Zeichenanforderungen (XSelectInput()).
   5. Das Fenster sichtbar machen mit XMapWindow().
   6. Ereignisse in einer Schleife abfragen und darauf reagieren (XNextEvent()).
   7. Am Ende Ressourcen freigeben und aufräumen (XDestroyWindow(), XCloseDisplay()   usw.).
  ```
2. **How does the `Expose` event trigger your drawing code?**
 ```
Wenn das Fenster sichtbar wird (z. B. durch Anzeigen oder Vergrößern), löst X11 ein Expose-Event aus.
In der Event-Schleife wird dann mit XDrawRectangle() das Rechteck neu gezeichnet, damit es immer sichtbar bleibt.
 ```
...
```
GTK bindet Ereignisse objektorientiert an Signale mit Callbacks, was die Handhabung vereinfacht. X11 erfordert eine manuelle Event-Loop, bei der jedes Ereignis selbst geprüft wird.
```
2. **Why use `pkg-config` when compiling GTK applications?**
```
pkg-config liefert automatisch Compiler- und Linker-Flags, wie Bibliothekspfade, was den Build-Prozess erleichtert und Fehler reduziert
```
